...
public class TestGameManager : MonoBehaviour
{
    ...
    private void KeyInput()
    {
        ...
        {
            NowKeyInput = KeyCode.LeftControl;
        ...
        else if (Input.GetKey(KeyCode.DownArrow) && OldKeyInput == NowKeyInput)
        {
            ...
            if (deltaTime > Define.conditionTime) // deltatime 값이 조건시간을 초과하면 아래 코드 진행. 
            {
                if (CheckDirection(PosY - 1, PosX))
                {
                    --PosY;
    ...
    private void RotateTetromino()
    {
        ...
        else if (Spin(PosY, PosX, tempArr))
        {
            if (3 <= TetroDegree)
            {
                TetroDegree = 0;
